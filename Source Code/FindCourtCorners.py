...
# You can see intermediate output by creating this directory:
...
class CourtFinder(object):
    ...
    def FindCourtCorners(self, frame, file_output=0):
        ...
        contours, hier = cv2.findContours(court_mask,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
        max_area = 0;
        max_c = None;
        for c in contours:
        ...
        contours, hier = cv2.findContours(isect_mask,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE);
        if file_output:
...
if __name__ == '__main__':
    ...
    video_path_kyle = "../Stereo Clips/stereoClip1_Kyle.mov"
    video_path_megan = "../Stereo Clips/stereoClip1_Megan.mov"

    vr = VideoReader(filename=video_path_kyle)
    ret, frame = vr.readFrame()

    if ret:
        finder = CourtFinder()
        finder.FindCourtCorners(frame, 1)
